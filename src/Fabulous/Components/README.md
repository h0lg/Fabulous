...
This is an attempt at making re-executable computation expressions (CE) with a context being passed implicitly.

## History and constraints
Today in Fabulous, there is only one source of truth for the whole app: its root state.
...
`Contextual` is a composable delegate that takes a `Context` (so we can pass it implicitly around the CE, mainly to be used
in `Bind` without making it visible in the user code) and returns a `Widget`, which is the typical body of a component.
...
The `Component` listens to its context's `Dirtied` event to know when to re-evaluate the body.
...
Behind the scenes, when calling `Bind` for firstName, `Context` will switch to the index `0`.
...
On subsequent reevaluations, `Context` switches back to index `0` to retrieve the values in order.
